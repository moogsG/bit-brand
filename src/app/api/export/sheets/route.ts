...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (type === "keywords") {
      ...
      const header = ["Keyword", "Monthly Volume", "Difficulty", "Intent", "Priority", "Cur. Position", "Target Position", "Status", "Notes"];
      ...
      const autoData = await getReportAutoData(clientId, report.month, report.year);
      ...
      const trafficSheet: string[][] = [
        ...
        [],
        ["Domain Rating", autoData.ahrefs.domainRating !== null ? String(autoData.ahrefs.domainRating) : ""],
        ["Backlinks", String(autoData.ahrefs.backlinks)],
        ["Referring Domains", String(autoData.ahrefs.referringDomains)],
        ["Organic Keywords", String(autoData.ahrefs.organicKeywords)],
      ];
    ...
    const spreadsheet = await createRes.json() as { spreadsheetId: string; spreadsheetUrl: string };
