...
export default async function AdminStrategyPage({
  ...
  const strategies = await db
    .select()
    .from(seoStrategies)
    .where(eq(seoStrategies.clientId, id))
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <AdminHeader title={`${client.name} — SEO Strategy`} />
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold">SEO Strategy</h1>
              <p className="text-sm text-muted-foreground">
                Manage strategy documents for {client.name}
              </p>
            </div>
            <AdminStrategyActions clientId={id} />
          </div>
        </div>
          ...
          <Card>
            <CardContent className="py-12 text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Create a new strategy document to get started.
              </p>
            </CardContent>
          </Card>
        ) : (
            ...
            {strategies.map((s) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[s.status ?? "DRAFT"]}`}>
                    {s.status ?? "DRAFT"}
                  </span>
                  <Link
                    href={`/admin/clients/${id}/strategy/${s.id}`}
