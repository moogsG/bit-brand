...
import { clients, clientUsers, users, invitations, dataSources } from "@/lib/db/schema";
...
export default async function ClientDetailPage({ params, searchParams }: PageProps) {
  ...
  const [clientUserRows, invitationRows, dataSourceRows] = await Promise.all([
    db
      .select({
        id: clientUsers.id,
  ...
  const tabs = ["overview", "data-sources", "sync", "users", "invitations"] as const;
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {activeTab === "sync" && (
          <SyncControls clientId={id} dataSources={dataSourceRows} />
        )}
                  ...
                  {clientUserRows.map((cu) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground">
                        Joined {cu.createdAt ? new Date(cu.createdAt).toLocaleDateString() : "—"}
                      </p>
                    </div>
                  ))}
                </div>
