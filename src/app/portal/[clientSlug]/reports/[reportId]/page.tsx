...
import { ArrowLeft, FileText, TrendingUp, Globe, Link2, Sparkles } from "lucide-react";
...
export default async function ReportDetailPage({
  ...
  const hasGa4 = autoData.ga4.totalSessions > 0 || autoData.ga4.dailyData.length > 0;
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Monthly Report</p>
          </div>
        ...
        <ReportExportButtons
          ...
          reportData={{
            ...
            backlinkProfile: {
              domainRating: autoData.ahrefs.domainRating,
              backlinks: autoData.ahrefs.backlinks,
              referringDomains: autoData.ahrefs.referringDomains,
              organicKeywords: autoData.ahrefs.organicKeywords,
            },
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {hasGa4 ? (
            <>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.ga4.totalSessions.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Sessions</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.ga4.totalUsers.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Users</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.ga4.totalPageviews.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Pageviews</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.ga4.totalOrganicSessions.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Organic Sessions</p>
                </div>
              </div>
              {autoData.ga4.dailyData.length > 0 && (
                <ReportTrafficChart
                  data={autoData.ga4.dailyData.map((d) => ({ date: d.date, value: d.organicSessions }))}
            ...
            <p className="text-sm text-muted-foreground">No traffic data available for {monthLabel}.</p>
          )}
        </CardContent>
      </Card>
          ...
          {hasGsc ? (
            <>
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.gsc.totalClicks.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Total Clicks</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.gsc.totalImpressions.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Impressions</p>
                </div>
                <div className="space-y-0.5">
                  <p className="text-2xl font-bold">{autoData.gsc.avgPosition.toFixed(1)}</p>
                  <p className="text-xs text-muted-foreground">Avg. Position</p>
                </div>
              </div>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">No search data available for {monthLabel}.</p>
          )}
        </CardContent>
      </Card>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    <th className="h-9 px-4 text-left align-middle font-medium text-muted-foreground">Query</th>
                    <th className="h-9 px-4 text-right align-middle font-medium text-muted-foreground">Clicks</th>
                    <th className="h-9 px-4 text-right align-middle font-medium text-muted-foreground">Impressions</th>
                    <th className="h-9 px-4 text-right align-middle font-medium text-muted-foreground">CTR</th>
                    <th className="h-9 px-4 text-right align-middle font-medium text-muted-foreground">Position</th>
                  </tr>
                </thead>
                <tbody>
                  {autoData.gsc.topQueries.map((q) => (
                    <tr key={q.query} className="border-b border-border last:border-0 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2.5 text-right">{(q.ctr * 100).toFixed(1)}%</td>
                      <td className="px-4 py-2.5 text-right">{q.position.toFixed(1)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="space-y-0.5">
                <p className="text-2xl font-bold">
                  {autoData.ahrefs.domainRating !== null ? Math.round(autoData.ahrefs.domainRating) : "—"}
                </p>
                <p className="text-xs text-muted-foreground">Domain Rating</p>
              </div>
              <div className="space-y-0.5">
                <p className="text-2xl font-bold">{autoData.ahrefs.backlinks.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Backlinks</p>
              </div>
              <div className="space-y-0.5">
                <p className="text-2xl font-bold">{autoData.ahrefs.referringDomains.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Referring Domains</p>
              </div>
              <div className="space-y-0.5">
                <p className="text-2xl font-bold">{autoData.ahrefs.organicKeywords.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Organic Keywords</p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No backlink data available for {monthLabel}.</p>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent>
          {hasAiVis && autoData.aiVisibility ? (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              <div className="space-y-0.5">
                <p className="text-2xl font-bold">
                  {autoData.aiVisibility.promptsVisible} / {autoData.aiVisibility.totalPromptsTested}
                </p>
                <p className="text-xs text-muted-foreground">Prompts Visible</p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No AI visibility data available for {monthLabel}.</p>
          )}
        </CardContent>
      </Card>
