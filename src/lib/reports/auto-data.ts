...
export async function getReportAutoData(
  ...
  const [ga4Rows, gscDailyRows, gscQueryRows, ahrefsRow, aiVisRow] = await Promise.all([
    // GA4 daily data
    db
      .select()
      .from(ga4Metrics)
    ...
    db
      .select({
        ...
        impressions: sql<number>`sum(${gscMetrics.impressions})`.as("impressions"),
    ...
    // Ahrefs — latest record in this month (or most recent before)
    db
      .select()
      .from(ahrefsMetrics)
      .where(
        and(
          eq(ahrefsMetrics.clientId, clientId),
          lte(ahrefsMetrics.date, endDate)
        )
      )
  ...
  const totalOrganicSessions = ga4Rows.reduce((s, r) => s + (r.organicSessions ?? 0), 0);
  ...
  const avgPosition =
    gscDailyRows.length > 0
      ? gscDailyRows.reduce((s, r) => s + r.avgPosition, 0) / gscDailyRows.length
      : 0;
  ...
  return {
    ...
    ahrefs: {
      domainRating: ahrefsRow?.domainRating ?? null,
      backlinks: ahrefsRow?.backlinks ?? 0,
      referringDomains: ahrefsRow?.referringDomains ?? 0,
      organicKeywords: ahrefsRow?.organicKeywords ?? 0,
    },
    aiVisibility: aiVisRow
      ? {
          overallScore: aiVisRow.overallScore,
          rankscaleScore: aiVisRow.rankscaleScore,
          semrushScore: aiVisRow.semrushScore,
