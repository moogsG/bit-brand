...
function trend(base: number, day: number, totalDays: number, growthPct: number): number {
  const progress = day / totalDays;
  return Math.round(base * (1 + progress * (growthPct / 100)) + (Math.random() - 0.5) * base * 0.12);
}
...
async function seedAdmin() {
  ...
  const [admin] = await db
    .insert(users)
    .values({ email: adminEmail, name: "BBA Admin", passwordHash, role: "ADMIN" })
    .returning();
...
async function seedDemoClient(adminId: string) {
  ...
  const existing = await db.select().from(clients).where(eq(clients.slug, slug)).get();
...
async function seedClientUser(clientId: string) {
  ...
  let clientUser = await db.select().from(users).where(eq(users.email, email)).get();
...
async function seedGA4(clientId: string) {
  ...
  for (let i = DAYS; i >= 0; i--) {
    ...
    const newUsers = Math.round(users * randFloat(0.55, 0.70, 3));
    const pageviews = Math.round(sessions * randFloat(2.8, 3.6, 3));
    const organicSessions = Math.round(sessions * randFloat(0.60, 0.72, 3));
...
const GSC_QUERIES = [
  { query: "best ecommerce platform", baseClicks: 62, baseImpr: 1800, baseCtr: 0.034, basePos: 8.2 },
  { query: "buy widgets online", baseClicks: 95, baseImpr: 2200, baseCtr: 0.043, basePos: 5.4 },
  { query: "acmecorp reviews", baseClicks: 145, baseImpr: 1100, baseCtr: 0.132, basePos: 2.1 },
  { query: "affordable widget supplier", baseClicks: 38, baseImpr: 980, baseCtr: 0.039, basePos: 11.3 },
  { query: "widget delivery australia", baseClicks: 72, baseImpr: 1650, baseCtr: 0.044, basePos: 6.7 },
  { query: "online widget store", baseClicks: 118, baseImpr: 3100, baseCtr: 0.038, basePos: 7.9 },
  { query: "custom widgets manufacturer", baseClicks: 29, baseImpr: 720, baseCtr: 0.040, basePos: 14.2 },
  { query: "widget comparison 2025", baseClicks: 54, baseImpr: 1420, baseCtr: 0.038, basePos: 9.1 },
  { query: "ecommerce seo tips", baseClicks: 21, baseImpr: 890, baseCtr: 0.024, basePos: 16.4 },
  { query: "product page seo", baseClicks: 17, baseImpr: 760, baseCtr: 0.022, basePos: 18.2 },
  { query: "bulk widget order", baseClicks: 83, baseImpr: 1980, baseCtr: 0.042, basePos: 5.8 },
  { query: "acme corp contact", baseClicks: 88, baseImpr: 560, baseCtr: 0.157, basePos: 1.8 },
  { query: "widget price list", baseClicks: 44, baseImpr: 1100, baseCtr: 0.040, basePos: 10.5 },
  { query: "free shipping widgets", baseClicks: 67, baseImpr: 1890, baseCtr: 0.035, basePos: 8.8 },
  { query: "eco friendly products online", baseClicks: 32, baseImpr: 940, baseCtr: 0.034, basePos: 13.1 },
  { query: "australian ecommerce shop", baseClicks: 57, baseImpr: 1560, baseCtr: 0.037, basePos: 9.4 },
  { query: "widget installation guide", baseClicks: 41, baseImpr: 1240, baseCtr: 0.033, basePos: 11.7 },
  { query: "same day delivery widgets", baseClicks: 28, baseImpr: 820, baseCtr: 0.034, basePos: 15.6 },
  { query: "business widget solutions", baseClicks: 19, baseImpr: 640, baseCtr: 0.030, basePos: 19.3 },
  { query: "widget warranty policy", baseClicks: 35, baseImpr: 880, baseCtr: 0.040, basePos: 7.2 },
];
...
async function seedGSC(clientId: string) {
  ...
  for (let i = DAYS; i >= 0; i--) {
    ...
    for (const q of GSC_QUERIES) {
      ...
      const growthMult = 1 + progress * 0.20; // +20% growth over 90 days
      const clicks = Math.max(0, Math.round(q.baseClicks * growthMult * randFloat(0.7, 1.3, 3)));
      const impressions = Math.max(1, Math.round(q.baseImpr * growthMult * randFloat(0.8, 1.2, 3)));
      const ctr = parseFloat((clicks / impressions).toFixed(4));
      const position = Math.max(1, parseFloat((q.basePos * randFloat(0.88, 1.12, 2)).toFixed(1)));

      rows.push({ clientId, date, query: q.query, clicks, impressions, ctr, position });
    }
  }
...
// ─── Ahrefs metrics (last 90 days) ───────────────────────────────────────────

async function seedAhrefs(clientId: string) {
  const DAYS = 90;
  const rows = [];
  ...
  for (let i = DAYS; i >= 0; i--) {
    const date = dateStr(i);
    const progress = (DAYS - i) / DAYS;
    const dr = parseFloat((35 + progress * 7 + (Math.random() - 0.5) * 1.2).toFixed(1));
    ...
    rows.push({
      ...
      date,
      domainRating: dr,
      urlRating: parseFloat((dr * randFloat(0.65, 0.80, 2)).toFixed(1)),
      backlinks,
      referringDomains,
      organicKeywords,
      organicTraffic,
  ...
  for (const row of rows) {
    await db.insert(ahrefsMetrics).values(row).onConflictDoNothing();
  }

  console.log(`✅ Ahrefs metrics seeded (${rows.length} rows)`);
}
  ...
  const KEYWORDS = [
    {
      ...
      monthlyVolume: 2900, difficulty: 52,
      intent: "COMMERCIAL" as const, priority: "HIGH" as const,
      currentPosition: 11, targetPosition: 5,
    ...
    },
    {
      ...
      monthlyVolume: 1600, difficulty: 38,
      intent: "TRANSACTIONAL" as const, priority: "HIGH" as const,
      currentPosition: 5, targetPosition: 2,
    ...
    },
    {
      ...
      monthlyVolume: 880, difficulty: 28,
      intent: "COMMERCIAL" as const, priority: "HIGH" as const,
      currentPosition: 12, targetPosition: 5,
    ...
    },
    {
      ...
      monthlyVolume: 1200, difficulty: 33,
      intent: "TRANSACTIONAL" as const, priority: "MEDIUM" as const,
      currentPosition: 7, targetPosition: 3,
    ...
    },
    {
      ...
      monthlyVolume: 590, difficulty: 44,
      intent: "COMMERCIAL" as const, priority: "MEDIUM" as const,
      currentPosition: null, targetPosition: 10,
    ...
    },
    {
      ...
      monthlyVolume: 1900, difficulty: 61,
      intent: "INFORMATIONAL" as const, priority: "MEDIUM" as const,
      currentPosition: 18, targetPosition: 10,
    ...
    },
    {
      ...
      monthlyVolume: 740, difficulty: 25,
      intent: "TRANSACTIONAL" as const, priority: "HIGH" as const,
      currentPosition: 6, targetPosition: 2,
    ...
    },
    {
      ...
      monthlyVolume: 1100, difficulty: 18,
      intent: "INFORMATIONAL" as const, priority: "LOW" as const,
      currentPosition: 4, targetPosition: 1,
    ...
    },
    {
      ...
      monthlyVolume: 680, difficulty: 30,
      intent: "COMMERCIAL" as const, priority: "MEDIUM" as const,
      currentPosition: null, targetPosition: 8,
    ...
    },
    {
      ...
      monthlyVolume: 420, difficulty: 22,
      intent: "TRANSACTIONAL" as const, priority: "MEDIUM" as const,
      currentPosition: 9, targetPosition: 4,
    ...
    },
    {
      ...
      monthlyVolume: 390, difficulty: 12,
      intent: "NAVIGATIONAL" as const, priority: "LOW" as const,
      currentPosition: 2, targetPosition: 1,
    ...
    },
    {
      ...
      monthlyVolume: 2200, difficulty: 58,
      intent: "INFORMATIONAL" as const, priority: "LOW" as const,
      currentPosition: null, targetPosition: 15,
    ...
    },
    {
      ...
      monthlyVolume: 1400, difficulty: 41,
      intent: "TRANSACTIONAL" as const, priority: "HIGH" as const,
      currentPosition: 8, targetPosition: 3,
    ...
    },
    {
      ...
      monthlyVolume: 310, difficulty: 14,
      intent: "INFORMATIONAL" as const, priority: "LOW" as const,
    ...
    },
    {
      ...
      monthlyVolume: 520, difficulty: 47,
      intent: "COMMERCIAL" as const, priority: "MEDIUM" as const,
      currentPosition: null, targetPosition: 8,
...
async function seedAIVisibility(clientId: string) {
  ...
  for (let m = 5; m >= 0; m--) {
    ...
    const overallScore = parseFloat((45 + progress * 20 + (Math.random() - 0.5) * 4).toFixed(1));
    const rankscaleScore = parseFloat((overallScore * randFloat(0.88, 1.02, 3)).toFixed(1));
    ...
    rows.push({
      ...
      overallScore,
      rankscaleScore,
      semrushScore: parseFloat((overallScore * randFloat(0.90, 1.05, 3)).toFixed(1)),
  ...
  for (let m = 5; m >= 0; m--) {
    ...
    for (const prompt of PROMPTS) {
      for (const platform of PLATFORMS) {
        ...
        const visibilityScore = isVisible ? parseFloat((60 + Math.random() * 35).toFixed(1)) : parseFloat((Math.random() * 30).toFixed(1));
