...
import type { DataSource, SyncJob } from "@/lib/db/schema";
import { SyncJobsDisplay } from "./sync-jobs-display";
				...
				<div className="min-w-0">
					...
					{state.status === "success" && state.result && (
						<p className="text-xs text-green-600 mt-0.5">
							Synced — {state.result.rowsInserted} row
							{state.result.rowsInserted === 1 ? "" : "s"} inserted
						</p>
					)}
			...
			<Card>
				...
				<CardContent>
					<p className="text-sm text-muted-foreground text-center py-4">
						No data sources configured. Add data sources in the Data Sources
						tab.
					</p>
				</CardContent>
			</Card>
